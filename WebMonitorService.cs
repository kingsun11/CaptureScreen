...
namespace CaptureScreen
{
    class WebMonitorService
    {
        ...
        public delegate byte[] OnImageCapture();
        private OnImageCapture capturerImageByte = null;

        ImageCapture screenUtil = new ImageCapture();

        public WebMonitorService(int serverPort)
        {
            _socket.Bind(new IPEndPoint(IPAddress.Any, serverPort));
            _socket.Listen(100);
        }

        public void start(OnImageCapture onImgCapute)
        {
            capturerImageByte = onImgCapute;
        ...
        private void HomePage(Socket response)
        {
        ...
        private void captureImage(Socket response)
        {
            string statusline = "HTTP/1.1 200 OK\r\n";   //状态行
            byte[] statusline_to_bytes = Encoding.UTF8.GetBytes(statusline);
            ...
            byte[] content_to_bytes = capturerImageByte();

            string header = string.Format("Content-Length: " + content_to_bytes.Length + "\r\nContent - Type:image/jpeg\r\nDate: " + string.Format("{0:R}", DateTime.Now) + "\r\nServer: Cftea Web Server\r\n");
            byte[] header_to_bytes = Encoding.UTF8.GetBytes(header);  //应答头
